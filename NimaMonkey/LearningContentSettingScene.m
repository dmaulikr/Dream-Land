...
@implementation LearningContentSettingScene
...
+(id)scene{
    ...
    LearningContentSettingScene *layer = [LearningContentSettingScene node];
...
-(void)addSceneTitle{
    ...
    CCLabelTTF *currentSceneTitle = [CCLabelTTF labelWithString:@"训练内容设置界面" fontName:@"ChalkboardSE-Bold" fontSize:40];
...
-(void)addSettingMenu{
    ...
    CCMenuItem *defaultSettingItem = [CCMenuItemImage itemWithNormalImage:@"button_defaultsetting-ipad.png" selectedImage:nil target:self selector:@selector(useDefaultSetting)];
    defaultSettingItem.position =ccp(screenSize.width*0.5,screenSize.height*0.7);
    
    CCMenuItem *MITrainingItem = [CCMenuItemImage itemWithNormalImage:@"button_MItraining-ipad.png" selectedImage:nil target:self selector:@selector(MITrainingSetting)];
    MITrainingItem.position =ccp(screenSize.width*0.5,screenSize.height*0.5);
    
    //创建菜单
    
    CCMenu *menu =[CCMenu menuWithItems:defaultSettingItem, MITrainingItem, nil];
    menu.position = CGPointZero;
    [self addChild:menu z:2];
...
-(void)useDefaultSetting{
    ...
    UIAlertView *alert = [[UIAlertView alloc]initWithTitle:@"默认设置" message:@"您选择了默认设置" delegate:self cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
...
-(void)MITrainingSetting{
    
    UIAlertView *alert = [[UIAlertView alloc]initWithTitle:@"内容设置" message:@"选择进行具体的多元智能内容共设置" delegate:self cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
